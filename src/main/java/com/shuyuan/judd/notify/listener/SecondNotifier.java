...
@Slf4j
@Service
@RocketMQMessageListener(topic = "${mq-topic.result-notify-r1}", consumerGroup = "SecondNotifier",consumeMode = ConsumeMode.CONCURRENTLY)
public class SecondNotifier implements RocketMQListener<String> {

    @Value("${mq-topic.result-notify-r2}")
    private String resultNotifyR2;
    ...
    private void send2NextTopic(JSONObject notification, String url){
        ...
        Message msg = new Message(resultNotifyR2, null, null, notification.toString().getBytes());
        msg.setDelayTimeLevel(6);
        rocketMQTemplate.asyncSend(resultNotifyR2, msg, new SendCallback() {
            @Override
